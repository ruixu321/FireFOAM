...
void OpenSMOKE_PDF_Flamelet::Read(const string file_name, const int index, const double chi_st)
{
	...
	// 3  - temperature (favre) (J/kg): T
	// 4  - density (reynolds) (kg/m3): rho
	// 5  - specific enthalpy (favre) (J/kg): h
	// 6  - molecular weight (favre) (kg/kmol): mw
	// 7  - specific heat (favre) (J/kg/K): cp
	// 8 - thermal conductivity (favre) (J/s/m/K): lambda
	// 9 - dynamic viscosity (favre) (kg/m/s): mu

    // 10 - absorption coeffcient (reynolds) (1/m): kappa1
    // 11 - absorption coeffcient (reynolds) (1/m): kappa2
    // 12 - absorption coeffcient (reynolds) (1/m): kappa3
    // 13 - absorption coeffcient (reynolds) (1/m): kappa4

    // 14 - Radiative directional emission (reynolds) (W/m3/sr): Em1
    // 15 - Radiative directional emission (reynolds) (W/m3/sr): Em2
    // 16 - Radiative directional emission (reynolds) (W/m3/sr): Em3
    // 17 - Radiative directional emission (reynolds) (W/m3/sr): Em4

    // 18 - Heat release rate (reynold): SumMH (W/m3)
    // 19 - molarfraction-N2
    // 20 - molarfraction-O2
	// 21 - species mass fractions
	...
	if (tag != "FlameMaster:4.0"){
		ErrorMessage("Only version 4.0 of FlameMaster is supported...");
	}
	...
	// mixture fraction + number of points
    ...
    _kappa_r1.resize(_n_variance+1);   // Add by Rui to have 5 bands WSGG 07/31/19
    _kappa_r2.resize(_n_variance+1);
    _kappa_r3.resize(_n_variance+1);
    _kappa_r4.resize(_n_variance+1);

    _Em_r1.resize(_n_variance+1);
    _Em_r2.resize(_n_variance+1);
    _Em_r3.resize(_n_variance+1);
    _Em_r4.resize(_n_variance+1);

    _SumMH_r.resize(_n_variance+1);
    // _ProdProgVar_r.resize(_n_variance+1);
    _MolarFracN2_f.resize(_n_variance+1);
    _MolarFracO2_f.resize(_n_variance+1);
	...
	for(j=1;j<=_n_variance;j++)
	{
        ...
        _kappa_r1[j].resize(_n_csi+1);
        _kappa_r2[j].resize(_n_csi+1);
        _kappa_r3[j].resize(_n_csi+1);
        _kappa_r4[j].resize(_n_csi+1);

        _Em_r1[j].resize(_n_csi+1);
        _Em_r2[j].resize(_n_csi+1);
        _Em_r3[j].resize(_n_csi+1);
        _Em_r4[j].resize(_n_csi+1);

        _SumMH_r[j].resize(_n_csi+1);
        _MolarFracN2_f[j].resize(_n_csi+1);
        _MolarFracO2_f[j].resize(_n_csi+1);
        // _ProdProgVar_r[j].resize(_n_csi+1);
...
// Added by Rui below to have WSGG 5 bands

    fInput >> tag;  // 10
    if (tag != "kappa1")
        ErrorMessage("Expected: kappa1, Found: " + tag);
    for(j=1;j<=_n_variance;j++)
        for(i=1;i<=_n_csi;i++)
            fInput >> _kappa_r1[j][i];

    fInput >> tag;  // 11
    if (tag != "kappa2")
        ErrorMessage("Expected: kappa2, Found: " + tag);
    for(j=1;j<=_n_variance;j++)
        for(i=1;i<=_n_csi;i++)
            fInput >> _kappa_r2[j][i];

    fInput >> tag;  // 12
    if (tag != "kappa3")
        ErrorMessage("Expected: kappa3, Found: " + tag);
    for(j=1;j<=_n_variance;j++)
        for(i=1;i<=_n_csi;i++)
            fInput >> _kappa_r3[j][i];

    fInput >> tag;  // 13
    if (tag != "kappa4")
        ErrorMessage("Expected: kappa4, Found: " + tag);
    for(j=1;j<=_n_variance;j++)
        for(i=1;i<=_n_csi;i++)
            fInput >> _kappa_r4[j][i];


    fInput >> tag;  // 14
    if (tag != "Em1")
        ErrorMessage("Expected: Em1, Found: " + tag);
    for(j=1;j<=_n_variance;j++)
        for(i=1;i<=_n_csi;i++)
            fInput >> _Em_r1[j][i];
    ...
    fInput >> tag;  // 15
    if (tag != "Em2")
        ErrorMessage("Expected: Em2, Found: " + tag);
    for(j=1;j<=_n_variance;j++)
        for(i=1;i<=_n_csi;i++)
            fInput >> _Em_r2[j][i];
    ...
    fInput >> tag;  // 16
    if (tag != "Em3")
        ErrorMessage("Expected: Em3, Found: " + tag);
    for(j=1;j<=_n_variance;j++)
        for(i=1;i<=_n_csi;i++)
            fInput >> _Em_r3[j][i];
    ...
    fInput >> tag;  // 17
    if (tag != "Em4")
        ErrorMessage("Expected: Em4, Found: " + tag);
    for(j=1;j<=_n_variance;j++)
        for(i=1;i<=_n_csi;i++)
            fInput >> _Em_r4[j][i];
    ...
    fInput >> tag;  // 18
    if (tag != "SumMH")
        ErrorMessage("Expected: SumMH, Found: " + tag);
    for(j=1;j<=_n_variance;j++)
        for(i=1;i<=_n_csi;i++)
            fInput >> _SumMH_r[j][i];
    ...
    fInput >> tag;  // 19
    if (tag != "MolaFrc_N2")
        ErrorMessage("Expected: MolaFrc_N2, Found: " + tag);
    for(j=1;j<=_n_variance;j++)
        for(i=1;i<=_n_csi;i++)
            fInput >> _MolarFracN2_f[j][i];

    fInput >> tag;  // 20
    if (tag != "MolaFrc_O2")
        ErrorMessage("Expected: MolaFrc_O2, Found: " + tag);
    for(j=1;j<=_n_variance;j++)
        for(i=1;i<=_n_csi;i++)
            fInput >> _MolarFracO2_f[j][i];

    // fInput >> tag;  // 19
    // if (tag != "ProdProgVar")
    //     ErrorMessage("Expected: ProdProgVar, Found: " + tag);
    // for(j=1;j<=_n_variance;j++)
    //     for(i=1;i<=_n_csi;i++)
    //         fInput >> _ProdProgVar_r[j][i];
...
void OpenSMOKE_PDF_Flamelet::GetMeanValues(const double csi, const double csiv2, vector<double>& extracted)
{
	...
	// Enthalpy [J/kg]
	extracted[3]	= (	_enthalpy_f[jCsiv2][iCsi]*q11   + _enthalpy_f[jCsiv2][iCsi+1]*q21 +
	          			_enthalpy_f[jCsiv2+1][iCsi]*q12 + _enthalpy_f[jCsiv2+1][iCsi+1]*q22 ) / q;

	// Molecular weight [kg/kmol]
	extracted[4]	= (	_mw_f[jCsiv2][iCsi]*q11   + _mw_f[jCsiv2][iCsi+1]*q21 +
	          			_mw_f[jCsiv2+1][iCsi]*q12 + _mw_f[jCsiv2+1][iCsi+1]*q22 ) / q;

	// Specific heat [J/kg/K]
	extracted[5]	= (	_cp_f[jCsiv2][iCsi]*q11   + _cp_f[jCsiv2][iCsi+1]*q21 +
	          			_cp_f[jCsiv2+1][iCsi]*q12 + _cp_f[jCsiv2+1][iCsi+1]*q22 ) / q;

	// Thermal conductivity [J/s/m/K]
	extracted[6]	= (	_lambda_f[jCsiv2][iCsi]*q11   + _lambda_f[jCsiv2][iCsi+1]*q21 +
	          			_lambda_f[jCsiv2+1][iCsi]*q12 + _lambda_f[jCsiv2+1][iCsi+1]*q22 ) / q;
	          		
	// Dynamic Viscosity [kg/m/s]
	extracted[7]	= (	_mu_f[jCsiv2][iCsi]*q11   + _mu_f[jCsiv2][iCsi+1]*q21 +
						_mu_f[jCsiv2+1][iCsi]*q12 + _mu_f[jCsiv2+1][iCsi+1]*q22 ) / q;   

	// Thermal diffusivity [m2/s]
	extracted[8]	= (	_alpha_f[jCsiv2][iCsi]*q11   + _alpha_f[jCsiv2][iCsi+1]*q21 +
						_alpha_f[jCsiv2+1][iCsi]*q12 + _alpha_f[jCsiv2+1][iCsi+1]*q22 ) / q;   	

																  		
    
    // absorption coefficient reynolds [1/m]
    extracted[9]    = ( _kappa_r1[jCsiv2][iCsi]*q11   + _kappa_r1[jCsiv2][iCsi+1]*q21 +
                        _kappa_r1[jCsiv2+1][iCsi]*q12 + _kappa_r1[jCsiv2+1][iCsi+1]*q22 ) / q;

    extracted[10]   = ( _kappa_r2[jCsiv2][iCsi]*q11   + _kappa_r2[jCsiv2][iCsi+1]*q21 +
                        _kappa_r2[jCsiv2+1][iCsi]*q12 + _kappa_r2[jCsiv2+1][iCsi+1]*q22 ) / q;

    extracted[11]   = ( _kappa_r3[jCsiv2][iCsi]*q11   + _kappa_r3[jCsiv2][iCsi+1]*q21 +
                        _kappa_r3[jCsiv2+1][iCsi]*q12 + _kappa_r3[jCsiv2+1][iCsi+1]*q22 ) / q;

    extracted[12]   = ( _kappa_r4[jCsiv2][iCsi]*q11   + _kappa_r4[jCsiv2][iCsi+1]*q21 +
                        _kappa_r4[jCsiv2+1][iCsi]*q12 + _kappa_r4[jCsiv2+1][iCsi+1]*q22 ) / q;


    // Radiative directional emission [W/m3/sr]
    extracted[13]   = ( _Em_r1[jCsiv2][iCsi]*q11   + _Em_r1[jCsiv2][iCsi+1]*q21 +
                        _Em_r1[jCsiv2+1][iCsi]*q12 + _Em_r1[jCsiv2+1][iCsi+1]*q22 ) / q;

    extracted[14]   = ( _Em_r2[jCsiv2][iCsi]*q11   + _Em_r2[jCsiv2][iCsi+1]*q21 +
                        _Em_r2[jCsiv2+1][iCsi]*q12 + _Em_r2[jCsiv2+1][iCsi+1]*q22 ) / q;

    extracted[15]   = ( _Em_r3[jCsiv2][iCsi]*q11   + _Em_r3[jCsiv2][iCsi+1]*q21 +
                        _Em_r3[jCsiv2+1][iCsi]*q12 + _Em_r3[jCsiv2+1][iCsi+1]*q22 ) / q;

    extracted[16]   = ( _Em_r4[jCsiv2][iCsi]*q11   + _Em_r4[jCsiv2][iCsi+1]*q21 +
                        _Em_r4[jCsiv2+1][iCsi]*q12 + _Em_r4[jCsiv2+1][iCsi+1]*q22 ) / q;


    // Heat Release Rate [W/m3]
    extracted[17]   = ( _SumMH_r[jCsiv2][iCsi]*q11   + _SumMH_r[jCsiv2][iCsi+1]*q21 +
                        _SumMH_r[jCsiv2+1][iCsi]*q12 + _SumMH_r[jCsiv2+1][iCsi+1]*q22 ) / q;

   	// Molar fraction of N2 
    extracted[18]   = ( _MolarFracN2_f[jCsiv2][iCsi]*q11   + _MolarFracN2_f[jCsiv2][iCsi+1]*q21 +
                        _MolarFracN2_f[jCsiv2+1][iCsi]*q12 + _MolarFracN2_f[jCsiv2+1][iCsi+1]*q22 ) / q;

  	// Molar fraction of O2 
    extracted[19]   = ( _MolarFracO2_f[jCsiv2][iCsi]*q11   + _MolarFracO2_f[jCsiv2][iCsi+1]*q21 +
                        _MolarFracO2_f[jCsiv2+1][iCsi]*q12 + _MolarFracO2_f[jCsiv2+1][iCsi+1]*q22 ) / q;


    // Production rate of progress variable [kg/s/m3]
    // extracted[16]    = ( _ProdProgVar_r[jCsiv2][iCsi]*q11   + _ProdProgVar_r[jCsiv2][iCsi+1]*q21 +
    //                     _ProdProgVar_r[jCsiv2+1][iCsi]*q12 + _ProdProgVar_r[jCsiv2+1][iCsi+1]*q22 ) / q;
