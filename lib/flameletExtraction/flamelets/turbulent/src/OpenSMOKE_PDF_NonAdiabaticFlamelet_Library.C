...
OpenSMOKE_PDF_NonAdiabaticFlamelet_Library::OpenSMOKE_PDF_NonAdiabaticFlamelet_Library()
{
	...
	cout << "The constructor of OpenSMOKE_PDF_NonAdiabaticFlamelet_Library is called" << endl;
}
...
void OpenSMOKE_PDF_NonAdiabaticFlamelet_Library::SetSpeciesToExtract(const string list_of_names)   	// This function is called and this function will do something to call the above function with the same name with different input type
{
	vector<string> names;
...
void OpenSMOKE_PDF_NonAdiabaticFlamelet_Library::Read()
{
	...
	cout << "_name variable" << _name << endl;				// Minh printing for more understanding
	ifstream fInput;
	fInput.open(_name.c_str(), ios::in);					// _name.c_str() value: PDF-Library/LookUpTable.Out ios::in is a mode when open a file, we have a lot of mode, ios::in is assumed like defaut mode
	...
	_jPhiAdiabatic = 0;				// Finding so thu tu cua adiabatic flame
	for(i=1;i<=_nphi;i++)
	...
	flamelet_libraries = new OpenSMOKE_PDF_Flamelet_Library[_nphi+1]; // flamelet_libraries objects of OpenSMOKE_PDF_Flamelet_Library
	for(i=1;i<=_nphi;i++)
	{

		string path = _path_library + "/" + _phi_names[i]; 	// path value: PDF-Library/PDF100 PDF-Library/PDF0 ...
		...
		flamelet_libraries[i].Read(); 	// flamelet_ object of OpenSMOKE_PDF_Flamelet 
...
void OpenSMOKE_PDF_NonAdiabaticFlamelet_Library::Summary()
{
	...
	cout << "     + The constructor of Number of Enthalpy defects:        " << _nphi << endl;
